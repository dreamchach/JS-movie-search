...
var searchBtnEl = document.querySelector("button.search");
var movieEl = document.querySelector("div.movies");
var moreBtnEl = document.querySelector("button.more");
var page = 1;
function getMovies(movie) {
  var page = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 1;
  var year = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : null;
  var type = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : "movie";
  _axios.default.get("https://www.omdbapi.com/?apikey=7035c60c&s=".concat(movie, "&page=").concat(page, "&y=").concat(year, "&type=").concat(type)).then(function (response) {
    console.log(response);
    var _iterator = _createForOfIteratorHelper(response.data.Search),
      _step;
    try {
      var _loop = function _loop() {
        var movieel = _step.value;
        console.log(movieel);
        ...
        el.addEventListener("click", function () {
          ...
          function getMovieInfo(id) {
            _axios.default.get("https://www.omdbapi.com/?apikey=7035c60c&i=".concat(id, "&plot=full")).then(function (response) {
              console.log(response);
              ...
              var productionEl = document.createElement("div");
...
function removeMoreBtn(movie, page, year, type) {
  _axios.default.get("https://www.omdbapi.com/?apikey=7035c60c&s=".concat(movie, "&page=").concat(page, "&y=").concat(year, "&type=").concat(type)).then(function (response) {
    ...
    var remove = response.data.totalResults;
...
removeMoreBtn("`${movie}`", "".concat(page), 2014, "`${type}`");
searchBtnEl.addEventListener("click", function () {
  var el = document.querySelectorAll("div.movie");
  for (var i = 0; i <= el.length - 1; i += 1) {
    console.log(i);
    el[i].parentNode.removeChild(el[i]);
  }
  moreBtnEl.style.display = "block";
  var typeEls = document.getElementById("type");
  var type = typeEls.value;
  console.log(type);
  var yearEls = document.getElementById("year");
  var year = yearEls.value;
  console.log(year);
  var movie = inputEl.value;
  console.log(movie);
  getMovies("\"".concat(movie, "\""), 1, "".concat(year), "\"".concat(type, "\""));
  removeMoreBtn("\"".concat(movie, "\""), 1, "".concat(year), "\"".concat(type, "\""));
  getId("\"".concat(movie, "\""), 1, "".concat(year), "\"".concat(type, "\""));
});
...
moreBtnEl.addEventListener("click", function () {
  ...
  var page = 1;
  page += 1;
  getMovies("\"".concat(movie, "\""), "".concat(page), "".concat(year), "\"".concat(type, "\""));
  removeMoreBtn("\"".concat(movie, "\""), "".concat(page), "".concat(year), "\"".concat(type, "\""));
  console.log(page);
});
function modal() {
  ...
  modal.querySelector("div.material-symbols-outlined").addEventListener("click", function () {
    bg.remove();
